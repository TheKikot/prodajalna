  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    if (!napaka1) {
      strankaIzRacuna(polja.seznamRacunov, function (napaka1, stranka) {
        if (!napaka1) {
          pesmiIzRacuna(polja.seznamRacunov, function(napaka1, pesmi) {
            if(!napaka1) {
              odgovor.setHeader('content-type', 'text/xml');
              odgovor.render('eslog',{vizualiziraj: true, postavkeRacuna: pesmi, stranka: stranka[0]});
            } else {
              odgovor.status(500).send('Prišlo je do napake!');
            }
          });
        } else {
          odgovor.status(500).send('Prišlo je do napake!');
...
// Vrni trenutno stranko
var trenutnaStranka = function(zahteva, callback) {
  pb.all("SELECT Customer.* FROM Customer WHERE Customer.CustomerId =" + zahteva.session.user,
    function(napaka, vrstice) {
      console.log(vrstice);
      callback(napaka, vrstice);
    });
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    var trenId = parseInt(zahteva.session.user, 10);
    if (!pesmi) {
      odgovor.sendStatus(500);
    ...
    vrniStranke(function(napaka1, stranke){
      console.log(stranke[trenId]);
      odgovor.setHeader('content-type', 'text/xml');
      odgovor.render('eslog', {
        vizualiziraj: zahteva.params.oblika == 'html' ? true : false,
        postavkeRacuna: pesmi, stranka: stranke[trenId - 1]
      });
    });
    }
  });
